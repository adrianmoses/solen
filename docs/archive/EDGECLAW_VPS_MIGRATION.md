# EdgeClaw — VPS Migration Spec

> Replacing the Cloudflare Workers / Durable Objects runtime with a tokio process,
> SQLite via sqlx, Docker Compose, and systemd on a Hetzner VPS.
...
| M5.6 | Credential store encrypts/decrypts via `ring` HKDF + `aes-gcm` | Round-trip test with real env key passes |
| M5.7 | `skill-web-search` container builds and responds to MCP tool calls | Search result returned via MCP |
| M5.8 | `docker compose up` starts all services, Caddy proxies correctly | Full stack running on local Docker |
| M5.9 | Multi-stage Dockerfile builds produce <30MB image | `docker images` confirms size |
| M5.10 | Deployed to Hetzner VPS, systemd service starts on boot | `systemctl status edgeclaw` shows active |
| M5.11 | Telegram message triggers agent turn end-to-end | Real Telegram message gets a real reply |
| M5.12 | TUI `edgeclaw setup` deploys via SSH without wrangler | Clean deploy from the wizard |
| M5.13 | `edgeclaw manage` status screen reads from `/admin/status` | Live data visible in TUI |
